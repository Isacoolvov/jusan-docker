...
sudo docker run -d -p 9999:80 --name jusan-docker-mount -v $(pwd)/jusan-docker-mount.conf:/etc/nginx/conf.d/default.conf:ro -v $(pwd)/jusan-docker-mount/:/var/www/example/ nginx:mainline
